        ...
        categories: [
            {
                ...
                programs: [
                    ...
                    { 
                        title: "채용검진&특수검진&비자검진", 
                        details: {
                            "안내사항": ["고객님의 목적에 따라 항목이 달라지므로, 예약 담당자와 상담 후 최종 확정됩니다."]
                        }
                    },
                    { 
                        title: "기본(공단검진 등)", 
                        details: {
                            "안내사항": ["국가건강검진 등 기본적인 검진입니다. 상담을 통해 원하시는 내역을 조율할 수 있습니다."]
                        }
                    },
                    { 
                        title: "단일항목(대장내시경만 등)", 
                        details: {
                            "안내사항": ["종합검진 외에 특정 항목만 원하시는 경우 선택해 주세요. 세부 사항은 상담 후 확정됩니다."]
                        }
                    }
                ]
            }
...
window.processChangeRequest = function() {
    ...
    if (request) {
        ...
        setTimeout(() => {
            window.appendMessage('coord', "요청하신 변경 사항을 접수했습니다. 담당 부서에서 확인 후 일정을 확정하면 이 화면에 다시 알려드리겠습니다.");
        }, 1000);
    }
};
...
// Dynamic Title Generator for realistic blog content
const generateTitle = (i, category) => {
    const topics = {
        '건강정보': [
            '한국 건강검진 필수 체크리스트', '외국인을 위한 KCD 질병 코드 안내', '금식 및 검진 전 준비사항 가이드',
            '내시경 검사 전후 주의사항', '건강검진 결과지 다국어 번역의 중요성', '한국 의료 시스템 완벽 가이드'
        ],
        '체킷소식': [
            '투명한 결제 시스템 업데이트', '새로운 다국어 번역 지원 언어 추가', '병원 예약 자동화 행정 시스템 도입',
            '프라이버시 보안 시스템 강화', '중간 마진 없는 100% 투명 플랫폼', '외국인 환자 데이터 관리 정책 개편'
        ],
        '고객후기': [
            '언어 장벽 없는 완벽한 행정 지원 경험', '복잡한 예약 과정을 한 번에 해결한 후기', '모국어로 결과지를 받아본 이야기',
            '수수료 없는 정가 결제로 신뢰감 상승', '베트남 고객의 서비스 이용 성공기', '가족 단위 방문객의 동행 기록'
        ],
        '한국생활': [
            '한국 입국 전 필수 비자 행정 가이드', '외국인 등록증 유무에 따른 검진 차이점', '한국 병원비 해외 카드 승인 팁',
            '검진 후 추천하는 서울 힐링 여행지', '한국 대중교통 및 병원 이동 안내', '방문 시 필요한 필수 서류 총정리'
        ]
    };
    const list = topics[category];
    const base = list[i % list.length];
    const suffixes = ['총정리', '완벽 가이드', '필독 사항', '알아보기', '행정 데이터', '이용 방법', '핵심 요약', '최신 정보'];
...
const generateContent = (title, category) => {
    ...
    if (category === '건강정보') {
        content += `<p>한국의 건강검진 시스템은 세계적으로 그 우수성을 인정받고 있습니다. 하지만 외국인 환자분들이 아무리 뛰어난 병원을 방문하더라도, 복잡한 의료 시스템과 언어 장벽 앞에서는 막막함을 느낄 수밖에 없습니다. 특히 이번 주제인 '${title}'와 관련해서는 사전에 정확한 정보를 숙지하고 준비하는 것이 성공적인 검진의 핵심입니다.</p>`;
        content += `<p>저희 CHECKIT은 단순한 의료 중개나 알선 업체가 아닙니다. 철저히 IT 플랫폼 기반으로 구축된 외국인 전용 비의료 행정 지원 소프트웨어(SaaS)로서, 환자분들이 병원 이용 시 겪는 행정적 어려움을 완벽하게 해소해 드립니다.</p>`;
        content += `<br><h3 style="color: #1a1a1a; font-size: 1.2rem; font-weight: 600; margin: 15px 0;">${title} - 핵심 체크포인트</h3>`;
        content += `<p>검진 전 금식 안내부터 당일 이동 동선, 그리고 가장 중요한 결과지 수령까지 모든 단계에서 세심한 관리가 필요합니다. 수많은 외국인들이 병원비와 중개 수수료가 합쳐진 불투명한 청구서를 받아왔지만, CHECKIT은 다릅니다. 저희는 고객이 병원에 직접 정가로 결제할 수 있도록 지원하며, 당사의 시스템 이용료만을 투명하게 분리하여 운영합니다.</p>`;
        content += `<p>특히 건강검진 후 발급되는 수십 장의 결과지에는 복잡한 의학 용어가 가득합니다. CHECKIT은 이를 국제 표준 질병 코드(KCD/ICD-10)에 맞추어 완벽하게 모국어로 번역해 드립니다. 이를 통해 고국에 돌아가서도 지속적인 건강 관리가 가능합니다.</p>`;
    } else if (category === '체킷소식') {
        content += `<p>저희 CHECKIT 플랫폼이 또 한 번의 중요한 업데이트를 맞이했습니다. 이번 <strong>'${title}'</strong> 업데이트를 통해 저희 서비스는 외국인 환자분들에게 한 차원 더 높은 수준의 비의료 행정 지원을 제공하게 되었습니다.</p>`;
        content += `<p>그동안 많은 에이전시들이 불투명한 마진을 남기며 수수료를 챙겨왔습니다. 그러나 CHECKIT은 의료법을 철저히 준수하는 IT 솔루션으로서, 중간 마진 없는 100% 투명한 정가 결제 시스템을 고집해 왔습니다. 이번 소식 역시 이러한 저희의 투명성 철학을 바탕으로 이루어진 결과입니다.</p>`;
        content += `<br><h3 style="color: #1a1a1a; font-size: 1.2rem; font-weight: 600; margin: 15px 0;">IT 기술로 혁신하는 행정 시스템</h3>`;
        content += `<p>대기 시간 없이 실시간으로 병원과 일정을 조율하고, 복잡한 문진표를 사전에 모국어로 번역하는 작업은 고도의 시스템이 뒷받침되어야 합니다. '${title}'의 도입으로 인해 예약부터 사후 관리까지의 프로세스가 더욱 자동화되고 안전해졌습니다.</p>`;
        content += `<p>저희는 앞으로도 고객의 프라이버시를 최우선으로 보호하며, 스마트폰 하나만 있으면 한국 병원을 자국 병원처럼 편안하게 이용할 수 있도록 지속적인 플랫폼 고도화를 진행할 것을 약속드립니다.</p>`;
    } else if (category === '고객후기') {
        content += `<p>실제 저희 플랫폼을 이용해주신 고객님의 생생한 경험을 공유해 드립니다. 이번 후기의 핵심 주제는 <strong>'${title}'</strong>입니다.</p>`;
        content += `<p>"처음 한국에서 검진을 결심했을 때 가장 두려웠던 것은 언어와 비용이었습니다. 하지만 CHECKIT을 통해 예약하는 순간부터 모든 걱정이 사라졌습니다." 많은 분들이 이렇게 말씀하십니다. 중개 수수료가 포함되지 않은 병원 공식 가격표를 그대로 확인하고 직접 결제할 수 있다는 점이 가장 큰 신뢰를 주었다고 합니다.</p>`;
        content += `<br><h3 style="color: #1a1a1a; font-size: 1.2rem; font-weight: 600; margin: 15px 0;">언어 장벽을 허문 완벽한 컨시어지</h3>`;
        content += `<p>이 고객님 역시 사전에 모국어로 번역된 문진표를 작성하고, 비자 발급에 필요한 예약확인증을 대사관 제출 요건에 맞게 완벽하게 지원받으셨습니다. 병원에 도착해서도 스마트폰을 통해 동선 안내를 받으며, 복잡한 대기 없이 수월하게 검사를 마치셨습니다.</p>`;
        content += `<p>무엇보다 검사 후 제공된 번역 결과지는 고객님께 큰 감동을 주었습니다. 어려운 의학 용어가 아니라, 본인이 완벽하게 이해할 수 있는 언어로 정리된 결과지와 표준 질병 코드를 통해 자신의 건강 상태를 명확히 파악할 수 있었기 때문입니다. 이것이 바로 CHECKIT이 추구하는 진정한 비의료 행정 서비스의 가치입니다.</p>`;
    } else {
        content += `<p>한국을 방문하는 외국인들에게 병원 이용은 가장 큰 난관 중 하나입니다. 이번에 다룰 <strong>'${title}'</strong> 주제는 한국에 체류하시거나 방문 예정인 분들이 반드시 알아두셔야 할 필수적인 생활 행정 정보입니다.</p>`;
        content += `<p>일부 국가의 경우 한국 단기 방문 시 비자가 필요하며, 건강검진을 목적으로 할 경우 병원에서 공식 발행한 명확한 예약확인증을 대사관에 제출해야 합니다. CHECKIT은 고객의 예약이 확정되는 즉시, 병원 원무과와 협의하여 영사관 제출용 요건에 부합하는 서류를 신속하게 발급해 드리는 행정 지원을 수행하고 있습니다.</p>`;
        content += `<br><h3 style="color: #1a1a1a; font-size: 1.2rem; font-weight: 600; margin: 15px 0;">안전하고 편리한 한국 방문 가이드</h3>`;
        content += `<p>한국 병원비 결제 시 해외 신용카드 승인 문제, 검사 전 금식으로 인한 식단 조절, 그리고 낯선 서울 지리에서의 대중교통 이동 방법까지. 아무리 사소해 보이는 문제라도 낯선 타국에서는 큰 스트레스가 될 수 있습니다.</p>`;
        content += `<p>저희 CHECKIT은 단순한 예약 대행을 넘어, 한국 도착 전부터 출국 시까지 필요한 모든 비의료적 가이드를 모국어로 제공합니다. 부풀려진 수수료나 잘못된 정보로 피해를 입지 마시고, 100% 투명하고 안전한 IT 플랫폼을 통해 한국의 우수한 의료 인프라를 경험해 보시길 권장합니다.</p>`;
    }
    
    content += `<br><p>더 자세한 정보나 궁금한 점이 있으시다면 언제든지 CHECKIT 마이페이지의 전담 매니저에게 문의해 주세요. 고객님의 언어로 가장 빠르고 정확하게 안내해 드리겠습니다. 감사합니다.</p>`;
...
for (let i = 21; i <= 207; i++) {
    ...
    const title = generateTitle(i, category);
    BLOG_SEED_DATA.push({
        id: 'post-' + i,
        title: title,
        category: category,
...
// Fix broken Unsplash URLs in the first 20 items and apply contextual, bright images
BLOG_SEED_DATA.forEach((post, index) => {
    if (index < 20) {
        post.thumbnail = 'https://loremflickr.com/600/400/' + getKeywords(post.title, post.category) + '?lock=' + (index + 900);
    }
});
