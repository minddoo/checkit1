document.addEventListener('DOMContentLoaded', () => {
    ...
    // ====================================================
    // [CHECKIT Platform] Firebase Auth & Admin Notification
    // ====================================================
    const firebaseConfig = {
        apiKey: "AIzaSy...", // Firebase Config
    ...
    if (typeof firebase !== 'undefined') {
        ...
        const sendAdminNotification = async (type, user, provider) => {
            const msg = `CHECKIT 신규 회원 활동 알림\n\n유형: ${type === 'SIGNUP' ? '회원가입' : '로그인'}\n이름: ${user.displayName || '이메일 회원'}\n이메일: ${user.email}\nUID: ${user.uid}\nProvider: ${provider}\n시간: ${new Date().toLocaleString('ko-KR')}`;
            
            try {
                await fetch('https://formspree.io/f/xaqdljye', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ email: user.email, message: msg })
                });
            } catch (e) { console.error('Notification Error:', e); }
        };

        // --- Firestore 사용자 동기화 ---
        const syncUserToFirestore = async (user, provider, isNewUser = false) => {
            ...
            if (isNewUser) {
                ...
                updateData.name = user.displayName || '이메일 회원';
                updateData.provider = provider;
                updateData.status = "가입완료";
        ...
        const showLoginModal = () => {
            ...
            if (!overlay) {
                const modalHtml = `
                    <div id="login-modal-overlay" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.85);z-index:3000;display:flex;justify-content:center;align-items:center;">
                        <div class="login-modal-box" style="background:#fff;padding:40px;border-radius:24px;width:90%;max-width:400px;text-align:center;position:relative;">
                            ...
                            <p id="login-guide-text" style="color:#666;margin-bottom:30px;font-size:0.9rem;">외국인 건강검진 행정 지원 플랫폼</p>
                ...
                const switchToSignup = () => {
                    ...
                    toggleMode.textContent = '이미 계정이 있으신가요? 로그인';
                };
                ...
                actionBtn.onclick = async () => {
                    ...
                    try {
                        if (isSignupMode) {
                            ...
                            await auth.signInWithEmailAndPassword(email, pw).then(res => {
                                syncUserToFirestore(res.user, 'email');
                                overlay.style.display = 'none';
                            });
                        }
                    } catch (e) {
                        if (e.code === 'auth/user-not-found') {
                            if (confirm('가입된 계정이 없습니다. 회원가입 하시겠습니까?')) switchToSignup();
